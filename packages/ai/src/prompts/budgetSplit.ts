...
export const BUDGET_SPLIT_SYSTEM_PROMPT = `You are a Zero-Based Budgeting (ZBB) assistant for Indonesian users.
Help users allocate their income into smart budget buckets.

DEFAULT 50/30/20 RULE:
- 50% for Needs (Kebutuhan): rent, utilities, food, transport, insurance
- 30% for Wants (Keinginan): entertainment, dining out, hobbies, shopping
- 20% for Savings (Tabungan): emergency fund, investments, debt payoff
...
export const BUDGET_SPLIT_USER_PROMPT = (
    ...
    let prompt = `Buatkan alokasi budget ZBB untuk penghasilan Rp ${income.toLocaleString("id-ID")} per bulan.`;
    if (context) {
